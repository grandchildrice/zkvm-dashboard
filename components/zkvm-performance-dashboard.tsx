  ...
  try {
    ...
    return rows.slice(1).map((row) => {
      ...
      headers.forEach((header, index) => {
        obj[header] = isNaN(parseFloat(row[index]))
          ? row[index]
...
export default function ZkVMPerformanceDashboard() {
  ...
  const renderChart = (data: any[], metric: string, title: string) => (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>
          Comparison across different zkVM implementations
        </CardDescription>
      </CardHeader>
      <CardContent>
  ...
  return (
    <div className="w-screen container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-6">Performance Comparisons</h1>
      <Tabs defaultValue="fibonacci">
        ...
        <TabsContent value="fibonacci" className="space-y-4">
          {renderChart(
            fibData,
            "proof_duration_sec",
            "100000th-Fibonacci - Prover Time (seconds)"
          )}
          {renderChart(
            fibData,
            "max_memory",
            "100000th-Fibonacci - Max Memory (GB)"
          )}
        </TabsContent>
        <TabsContent value="matrix" className="space-y-4">
          {renderChart(
            matrixData,
            "proof_duration_sec",
            "30x30MatrixOps - Prover Time (seconds)"
          )}
          {renderChart(
            matrixData,
            "max_memory",
            "30x30MatrixOps - Max Memory (GB)"
          )}
        </TabsContent>
      </Tabs>
    </div>
